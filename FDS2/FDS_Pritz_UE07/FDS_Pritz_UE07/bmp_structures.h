...
namespace graphics {
	...
	// pragma = komprimieren. char + int in struct würde so aussehen:
...
#pragma pack (pop)
	...
	// Nicht in pragma push 1 packen! (nicht komprimieren)
	// Macht keinen unterschied, da dies nur zur laufzeit relevant ist
	struct pixel_type {
		byte_type blue;
		byte_type green;
		byte_type red;
	...
	pixel_type const red = { 0, 0, 255 };
	pixel_type const green = { 0, 255, 0 };
	pixel_type const blue = { 255, 0, 0 };
	pixel_type const purple = { 255, 0, 255 };
	pixel_type const cyan = { 255, 255, 0 };
	pixel_type const yellow = { 0, 255, 255 };
	pixel_type const white = { 255, 255, 255 };
	pixel_type const black = { 0, 0, 0 };
}
#endif
