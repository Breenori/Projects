...
namespace graphics {
	enum background_type {BG_LIGHT, BG_DARK};
	...
	// Detects all objects in the given bitmap identified by the threshold and the background type.
	// Returns the amount of objects found.
	// WARNING: Number of objects can vary greatly because of limitations in recursion depth.
	size_t detect_objects(graphics::bitmap_type const& bmp, size_t const threshold, background_type const type);
	// Fills all preprocessed pixels adjacent to each other with the specified color.
	void floodfill(graphics::bitmap_type& bmp, size_t const threshold, int const x, int const y, graphics::pixel_type const& color);
	// Worker for the floodfill recursive function.
	void floodfill_worker(graphics::bitmap_type& bmp, size_t const threshold, int const x, int const y, graphics::pixel_type const& color, size_t const depth);
