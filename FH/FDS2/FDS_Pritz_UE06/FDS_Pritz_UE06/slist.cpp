...
slist::slist(size_t const n) : slist()
{
...
slist::value_t slist::get(size_t n) const
{
	// If index <n> out of bounds, return false.
	if (n > listsize)
	{
...
size_t slist::remove_all(value_t const& value)
{
	...
	else
	{
		...
		while (p_root != nullptr && p_root->data == value)
		{
			node_t* next = p_root->next;
			delete[] p_root;
			p_root = next;
...
void slist::copy_from(slist const& rhs)
{
	clear();
	if (rhs.p_root != nullptr)
	{
		node_t* head_ref = rhs.p_root;
		while (head_ref != nullptr)
		{
			this->push_back(head_ref->data);
			head_ref = head_ref->next;
		}
	}
}

void slist::init(size_t const n)
{
	clear();
