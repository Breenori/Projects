...
void ArithmeticParser::set_variable(std::string const& name, double const val)
{
	vars[name] = val;
}
bool ArithmeticParser::get_variable(std::string const& name, double& val)
{
	if (vars.count(name))
	{
		val = vars[name];
...
// Variable = any non operator/number string. (It however CAN contain numbers)
bool ArithmeticParser::is_tb_Variable() const
{
	return std::holds_alternative<std::string>(m_scanner.current_symbol().m_attr);
}
...
double ArithmeticParser::parse_Factor() {
	...
	{
		value = parse_Variable(); m_scanner.next_symbol();
	}
	else
	{
		throw ParserException("Error parsing 'Factor'.");
...
double ArithmeticParser::parse_Variable()
{
	...
	double value(0);
	std::variant<int, double, std::string> cur_val = m_scanner.current_symbol().m_attr;

	if (std::holds_alternative<std::string>(cur_val))
	{
		string name(std::get<std::string>(cur_val));

		if(!get_variable(name, value))
		{
			throw ParserException("Error parsing 'Variable'. Variable doesn't exist.");
		}
	}
